  ...
  useEffect(() => {
    if (isLoaded && user) {
      ...
      if (cloudEnrolledIds && Array.isArray(cloudEnrolledIds)) {
        ...
        if (mockUserMatch) {
          const userCourses = initialCourses.filter(c => 
      ...
      setCourseList(globalSeatCounts);
    }
  }, [isLoaded, user]);
        ...
        <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-8 p-8 -mt-12 text-left">
          ...
          <div className="bg-slate-900 p-8 rounded-[2.5rem] text-white shadow-xl">
              <p className="text-4xl font-black mb-1">
                {COURSE_CAPACITY - (liveCourseData?.enrolledCount || 0)}
              </p>
              <p className="text-slate-400 font-bold uppercase text-[10px] tracking-widest mb-6 text-emerald-400">Available Seats</p>
              <button 
                onClick={() => { enroll(activeCourse); setActiveCourse(null); }}
  ...
  return (
    <div className="min-h-screen bg-[#FDFDFF] text-slate-900 font-sans">
      ...
      <main className="max-w-7xl mx-auto p-6 md:p-10 grid grid-cols-1 lg:grid-cols-12 gap-10">
        ...
        <section className="lg:col-span-9">
          <h2 className="text-4xl font-black text-slate-900 mb-8 tracking-tighter italic text-left">COURSE CATALOG</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            {courseList.map((course) => {
              const theme = getTheme(course.category);
              const isEnrolled = enrolled.find(c => c.id === course.id);
